...
`1.1.3` 候选版本修正运行中升级后可能继续使用旧样式的问题，让新的紧凑弹窗样式立即加载。短标签使用紧凑宽度，长标签可适度扩宽，同时保留字号、行高和搜索。打开时保留列表焦点，点击搜索框后才开始输入；点击 `#标签` 或状态列应保留已有多选，后台列表刷新不应让正在使用的弹窗突然消失。隔离环境已复现旧样式缓存并验证加载修正，实际升级和界面效果仍需用户验收。
...
切换电脑前先提交并推送；另一台电脑开始工作前先拉取 `main`。详细合同见 [架构说明](docs/ARCHITECTURE.md)、[开发说明](docs/DEVELOPMENT.md) 和 [1.1.3 人工验收清单](docs/MANUAL_ACCEPTANCE_1.1.3.md)。`1.1.3` 为待验收版本，`1.1.0`、`1.1.1`、`1.1.2` 候选及其清单保留且不正式发布；正式发布前请继续使用 Releases 中的最新正式版本。
