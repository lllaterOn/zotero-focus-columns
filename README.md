...
在条目列表工具栏的“视图组”菜单中，将当前列布局另存为命名视图组，之后可直接切换，也可重命名、删除或调整组的排列。调整列后，需主动使用“更新当前视图”保存；临时调整不会自动覆盖已保存的组。切换只应用列布局，保留当前主排序、次级排序、搜索、筛选和条目选择，隐藏当前排序列也不改变排序。
...
切换电脑前先提交并推送；另一台电脑开始工作前先拉取 `main`。详细合同见 [架构说明](docs/ARCHITECTURE.md) 和 [开发说明](docs/DEVELOPMENT.md)。当前维护候选为 `1.2.0`，Draft XPI 须按 [1.2.0 人工验收清单](docs/MANUAL_ACCEPTANCE_1.2.0.md) 检查后才能正式发布；自动检查不能替代真实 Zotero 验收。[1.1.3 仍为已正式发布版本](https://github.com/lllaterOn/zotero-focus-columns/releases/tag/v1.1.3)，其 [验收与发布记录](docs/MANUAL_ACCEPTANCE_1.1.3.md) 保留。`1.1.0`、`1.1.1`、`1.1.2` 候选及其清单保留且不正式发布。
