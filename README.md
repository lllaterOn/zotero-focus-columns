...
`1.1.1` 候选版本继续改进弹窗交互：短标签使用紧凑宽度，长标签可适度扩宽；打开时保留列表焦点，点击搜索框后才开始输入。点击 `#标签` 或状态列应保留已有多选，后台列表刷新不应让正在使用的弹窗突然消失。这些界面行为仍需在真实 Zotero 中验收。
...
切换电脑前先提交并推送；另一台电脑开始工作前先拉取 `main`。详细合同见 [架构说明](docs/ARCHITECTURE.md)、[开发说明](docs/DEVELOPMENT.md) 和 [1.1.1 人工验收清单](docs/MANUAL_ACCEPTANCE_1.1.1.md)。`1.1.1` 为待验收版本，`1.1.0` 候选及其清单保留且不正式发布；正式发布前请继续使用 Releases 中的最新正式版本。
