...
For `1.2.4`, use [the current manual acceptance checklist](MANUAL_ACCEPTANCE_1.2.4.md). This metadata-only patch retains the existing runtime behavior and data contracts. Automated verification must confirm version consistency, repository hygiene, deterministic XPI assembly, and the packaged manifest name and description. The user explicitly authorized direct formal publication without a separate real-Zotero acceptance pass; record that limitation rather than marking unperformed checks as passed.
