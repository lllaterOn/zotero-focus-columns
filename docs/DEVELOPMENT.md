...
For `1.2.0`, use [the current manual acceptance checklist](MANUAL_ACCEPTANCE_1.2.0.md). Automated tests should cover layout application without sort or selection changes, preservation of unavailable columns, separate local widths, and the legacy/schema-2 settings boundary. A legacy settings snapshot with no group field must keep its old hash and must not delete local groups; an explicit empty list is a deliberate deletion. Test conflicts and backups through the existing settings channel, including clients that have not used groups.
